...
void 			SerialDevice::Open()
{
	...
	Receive(receiptBuffer->Data.buffer, maxReceiptLength);
}
...
{
	uint32_t receiptPosition = DMA_DATA_LENGTH(maxReceiptLength, adapter->DMAReceiver); //get current write position

	if (lastReceiptPosition == 0)
		receiptLength = (receiptPosition - lastReceiptPosition);
	else if(lastReceiptPosition == receiptPosition)
		receiptLength = maxReceiptLength;
	else
		receiptLength = (maxReceiptLength - lastReceiptPosition) + receiptPosition;
	...
	if (lastReceiptPosition >= maxReceiptLength)
...
void 			SerialDeviceCollection::Add(SerialDevice *serialDevice)
{
	int id = hashtableId(serialDevice->DeviceAddress(), 17);

	if (SerialDevices->Table[id] == NULL)
		SerialDevices->Table[id] = serialDevice;
	else
	{
		for (int i = 0; i < SerialDevices->Count; i++)
		{
			if (SerialDevices->Table[i] == NULL)
				SerialDevices->Table[i] = serialDevice;
		}
	}

	SerialDevices->Count++;
}
SerialDevice* 	SerialDeviceCollection::Get(uint32_t deviceAddress)
{
	int id = hashtableId(deviceAddress, 17);
	SerialDevice *result = SerialDevices->Table[id];
...
uint8_t 		SerialDeviceCollection::Remove(uint32_t deviceAddress)
{
	int id = hashtableId(deviceAddress, 17);
	SerialDevice *result = SerialDevices->Table[id];
...
SerialDeviceCollection* SerialDevices = new SerialDeviceCollection();
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	SerialDevice *sc = SerialDevices->Get((uint32_t)huart->Instance);
	sc->TransmitComplete();
}
