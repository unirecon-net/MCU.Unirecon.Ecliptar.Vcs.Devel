...
LightsController::LightsController(GPIO_TypeDef* frontLightPort, GPIO_TypeDef* rearLightPort)
{
	FrontLight.LightPort = frontLightPort;
	RearLight.LightPort = rearLightPort;
    attachController(4);
}
...
void        LightsController::assign(int controlSetId, uint16_t frontLightPin, ControlChannel* frontLightChannel, uint16_t rearLightPin, ControlChannel* rearLightChannel)
{
    for(int i = 0; i < 3; i++)
    {
    	FrontLightChannel[i] = NULL;
    	RearLightChannel[i] = NULL;
    }
