...
uint8_t RC_IBUS::pulseDepreciation(ControlChannel *channel)
{
	...
	{
		channel->pulseWidth = channel->innerPulseWidth;
...
float RC_IBUS::pulseToPercents(ControlChannel *channel)
{
	...
	return (pulse < midPulse) ?
			(((float) (midPulse - pulse) * 100.00F) / (float) (midPulse - minPulse)) * -1.00F :
			(((float) (pulse - midPulse) * 100.00F) / (float) (maxPulse - midPulse));
}
...
{
	uint8_t _size = size;
	while (_size > 3 && !frameStarted && data->Read())
	{
		uint8_t rbyte = data->ReadedByte();
		if (rbyte == CHANNELS_MSG_LENGTH &&
				size >= CHANNELS_MSG_LENGTH)
		{
			frameStarted = 1;
	...
	if (frameStarted && _size > 0)
	{
		...
		if (tocopy <= _size)
		{
			data->Read(&requestBuffer[0], tocopy);
			frameLength += tocopy;
			_size -= tocopy;
...
inline u8 RC_IBUS::ibusChecksum(uint8_t *buffer)
{
